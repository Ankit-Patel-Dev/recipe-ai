...
const preferenceInstructions = (
  ...
  const selected = [
    ...
    [
      'requested dish types',
      preferences.dishTypes,
    ],
  ].filter(
    ([, value]) => value?.length
  );
  ...
  if (!selected.length) {
    return 'No additional preferences were selected.';
  }
...
const normaliseRecipes = (
  ...
  if (!validRecipes.length) {
    throw new Error(
      'The AI did not return a usable recipe. Please try again.'
    );
  }
...
const requestRecipes = async (
  ...
  if (!activeApiKey) {
    throw new Error(
      'Missing API key. Please enter your Gemini API Key in the Profile tab.'
    );
  }
  ...
  if (imageBase64) {
    ...
    console.log(
      '[GEMINI] Base64 starts with:',
      normalized.base64.substring(
        0,
        30
      )
    );
  ...
  try {
    ...
    if (!rawResponse) {
      throw new Error(
        'Gemini returned no recipe. Please try again.'
      );
    }

    return normaliseRecipes(
      extractJSON(
        rawResponse
      )
    );
...
export const generateRecipeFromText =
  async (
    ingredients,
    preferences,
    signal
  ) => {
    const isLimitedInput =
...
${
  isLimitedInput
    ? 'The input is limited. For every option, suggest exactly 2 or 3 useful non-pantry ingredients in missingIngredients that would make the dish complete. Do not present a complete fake recipe using only the limited input.'
    : 'List only genuinely needed extra ingredients in missingIngredients; salt, oil, water, and basic spices may be treated as pantry staples.'
}
...
`;
    ...
    try {
      ...
      console.error(
        'Error generating recipe from text:',
        error
      );
...
export const generateRecipeFromImage =
  async (
    base64Image,
    preferences,
    signal
...
If the visible ingredients are insufficient for a complete recipe, missingIngredients must suggest 2 or 3 useful items to add; otherwise list only genuinely needed extras.

Salt, oil, water, and basic spices may be treated as pantry staples.
...
`;
    ...
    try {
      ...
      console.error(
        'Error generating recipe from image:',
        error
      );
...
export const generateRecipeByName =
  async (
    recipeName,
    signal
  ) => {
...
`;
      ...
      console.error(
        'Network or fetch error generating recipe by name:',
        error
      );
